...
export async function getUserAppointments(req: AuthRequest, res: Response): Promise<Response> {
  const paramId = Number(req.params.id);
  try {
    // get user's appointments
    const appointments = await db.getAppointments();
    const userAppointments = Object.values(appointments).filter((a) => a.userId === paramId);
...
export async function remove(req: AuthRequest, res: Response): Promise<Response> {
  try {
    const { id } = req.params;
...
export async function update(req: AuthRequest, res: Response): Promise<Response> {
  try {
    const { id } = req.params;
    const { patch } = req.body;
    if (!patch) {
      return res.status(400).json({ message: 'this endpoint requires a patch in the body' });
    }
    const updatedUser = await db.updateItem<AuthUser>(Number(id), db.filenames.users, patch);

    const user = removePasswordandAddToken(updatedUser);

    return res.status(200).json({ user });
  } catch (e) {
    return res.status(500).json({ message: `could not update appointment: ${e}` });
  }
}
...
export async function auth(req: Request, res: Response): Promise<Response> {
  ...
  const validUser = users.reduce(
    (foundUser: AuthUser | null, user) =>
      user.email === email && passwordIsValid(password, user) ? user : foundUser,
    null,
  );
